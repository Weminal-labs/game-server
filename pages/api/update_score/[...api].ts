...
import { UpdateScoreRequest, UpdateScoreResponse, UpdateScoreResult } from "@/lib/shared/interfaces";
...
const buildTx: GaslessTransactionBytesBuilder = async (req, { wallet }) => {
  const [error, body] = validate(req.body, UpdateScoreRequest);
  ...
  const gaslessTxBytes = await buildGaslessTransactionBytes({
    ...
    build: async (txb) => {
      txb.moveCall({
        target: `${MOVE_PACKAGE_ID}::wecastle::update_score`,
        arguments: [
          txb.pure.u64(body.score),
 ...
 */
const parseTxRes: TransactionResponseParser<UpdateScoreResponse> = async (_, txRes) => {
