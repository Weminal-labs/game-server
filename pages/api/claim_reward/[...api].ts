import { MOVE_PACKAGE_ID, MOVE_OBJECT_ID, MOVE_MISSION_ID } from "@/lib/api/move";
import { gas, sui } from "@/lib/api/shinami";
import { ClaimRewardRequest, ClaimRewardResponse, ClaimRewardResult } from "@/lib/shared/interfaces";
...
const buildTx: GaslessTransactionBytesBuilder = async (req, { wallet }) => {
  const [error, body] = validate(req.body, ClaimRewardRequest);
  ...
  const gaslessTxBytes = await buildGaslessTransactionBytes({
    ...
    build: async (txb) => {
      txb.moveCall({
        target: `${MOVE_PACKAGE_ID}::mission::claim_reward`,
        arguments: [
          txb.object(MOVE_MISSION_ID),
          txb.object(MOVE_OBJECT_ID),
          txb.pure.u64(body.id),
        ],
      });
    },
  });
 ...
 */
const parseTxRes: TransactionResponseParser<ClaimRewardResponse> = async (_, txRes) => {
